...
Enemy::Enemy(int _type)
{
	...
	else if (type == ON_TRACKRUN_ENEMY)
	{
		posX = rand() % (SCREEN_WIDTH + ENEMY_POSITION_RANGE) + SCREEN_WIDTH;
		posY = TRACK_RUN ;
	}
	else if (type == IN_AIR_1)
	{
		posX = rand() % (SCREEN_WIDTH + ENEMY_POSITION_RANGE) + SCREEN_WIDTH;
		posY = rand() % (500 - 400 + 1) + 10;
	}
...
void Enemy::loadFromFile(std::string path, SDL_Renderer* gRenderer)
{
	...
	else
	{
		...
		lcTexture = SDL_CreateTextureFromSurface(gRenderer, tmpSurface);
		if (lcTexture == nullptr)
		{
...
int Enemy::GetType()
{
	...
	else if (type == IN_AIR_2)
	{
		return IN_AIR_2;
	}
	else if (type == IN_AIR_3)
	{
		return IN_AIR_3;
	}
	else
	{
		return ON_TRACKRUN_ENEMY;
	}
}
